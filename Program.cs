...
class Program {
  ...
  public class AppModel : IDisposable {
    ...
    public AppModel() { _instance = this; }
    ...
    private readonly IResourceResolver _coreSource =
        new CachedResolver(ZipSource.CreateValidationSource());
...
#region QML - accessible properties
    ...
    public string ScopeDirectory {
      ...
      set {
        ...
        var directorySource = new CachedResolver(new DirectorySource(
            _scopeDirectory,
            new DirectorySourceSettings{IncludeSubDirectories = true}));

        // Finally, we combine both sources, so we will find profiles both from
        // the core zip as well as from the directory. By mentioning the
        // directory source first, anything in the user directory will override
        // what is in the core zip.
    ...
    private List<Issue>
    convertIssues(List<OperationOutcome.IssueComponent>issues) {
    ...
    private IPositionInfo
    GetPositionInfo(OperationOutcome.IssueComponent issue) {
      ...
      case ResourceFormat.Json:
        serializationDetails =
            elementWithError.First().GetJsonSerializationDetails();
        break;
      case ResourceFormat.Xml:
        serializationDetails =
            elementWithError.First().GetXmlSerializationDetails();
        break;
      case ResourceFormat.Unknown:
        return null;
      default:
    ...
    ///< summary>Trims Java FHIRpath of its position information, which isn't
    ///< always correct
    /// we have to compute it ourselves for .NET, might as well do it for
    /// Java</summary>
    ...
    public bool LoadResourceFile(string text) {
      ...
      filePath = filePath.RemovePrefix(
          RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? "file:///"
                                                              : "file://");
    ...
    public void copyToClipboard(string message) { Clipboard.SetText(message); }

    public void CopyValidationReportCsv() {
      using(var writer = new StringWriter()) using(var csv =
      ...
      List<MarkdownIssue>ConvertToMarkdown(List<Issue>rawIssues) {
        ...
        foreach(var issue in rawIssues) {
          markdownIssues.Add(new MarkdownIssue(){
              Severity = issue.Severity, Text = issue.Text,
              Location =
                  (issue.LineNumber == 0 && issue.LinePosition == 0)
                      ? ""
    ...
    public OperationOutcome ValidateWithDotnet(CancellationToken token) {
      ...
      try {
        var externalTerminology =
            new ExternalTerminologyService(new FhirClient(TerminologyService));
        var localTerminology =
            new LocalTerminologyService(_combinedSource ?? _coreSource);
        var summaryProvider =
            new Hl7.Fhir.Specification.StructureDefinitionSummaryProvider(
                _combinedSource ?? _coreSource);
        var combinedTerminology = new FallbackTerminologyService(
            localTerminology, externalTerminology);
        ...
        if (InstanceFormat == ResourceFormat.Xml) {
          ...
          _parsedResource =
              FhirJsonNode
                  .Parse(
                      ResourceText, settings
                      : new FhirJsonParsingSettings{AllowJsonComments = true})
                  .ToTypedElement(summaryProvider);
        } else {
        ...
        Console.WriteLine(
            $".NET validation performed in {sw.ElapsedMilliseconds}ms");
    ...
    private string SerializeResource(string resourceText,
                                     ResourceFormat instanceFormat) {
      var fileName =
          $"{Path.GetTempFileName()}.{(instanceFormat == ResourceFormat.Json ? " json
          " : " xml ")}";
    ...
    // in case the Java validator crashes (which it can if it doesn't like
    // something) it won't produce an OperationOutcome for us. Take what we've
    // got and make one ourselves
    private OperationOutcome ConvertJavaStdout(string output) {
      ...
      using(var reader = new StringReader(output)) {
        for (var line = reader.ReadLine(); line != null;
          ...
          result.Issue.Add(new OperationOutcome.IssueComponent{
              Severity = OperationOutcome.IssueSeverity.Error,
              Details = new CodeableConcept{Text = line},
    ...
    public OperationOutcome ValidateWithJava(CancellationToken token) {
      ...
      var resourcePath = SerializeResource(ResourceText, InstanceFormat);
            ...
            var scopeArgument = string.IsNullOrEmpty(ScopeDirectory)
                                    ? ""
                                    : $" -ig \"{ScopeDirectory}\"";
            var outputJson = $"{Path.GetTempFileName()}.json";
            var finalArguments =
                $"-jar {validatorPath} -version 3.0 -tx \"{TerminologyService}\"{scopeArgument} -output {outputJson} {resourcePath}";
              ...
              try {
                ...
                if (ex.Message == "The system cannot find the file specified") {
                  result.Issue.Add(new OperationOutcome.IssueComponent{
                      Severity = OperationOutcome.IssueSeverity.Error,
                      Diagnostics =
                          "Java could not be found. Is your Java installed and working correctly? See https://www.java.com/en/download/help/version_manual.xml",
                      Code = OperationOutcome.IssueType.Exception});
                } else {
                  result.Issue.Add(new OperationOutcome.IssueComponent{
                      Severity = OperationOutcome.IssueSeverity.Error,
                ...
                sw.Stop();
                Console.WriteLine(
                    $"Java validation performed in {sw.ElapsedMilliseconds}ms");
                return result;
              }
    ...
    public async void StartValidation() {
      ...
      Task<OperationOutcome>validateWithJava =
          Task.Run(() => ValidateWithJava(token), token);
      Task<OperationOutcome>validateWithDotnet =
          Task.Run(() => ValidateWithDotnet(token), token);
    ...
    public void CancelValidation() {
      ...
      foreach(var process in _validatorProcesses) { process.Kill(); }
    ...
    public async void CheckForUpdates() {
      Debug.WriteLine(
          $"Currently running Hammer v{ApplicationVersion}. Checking for updates...");
      ...
      if (latestVersion > currentVersion) {
        ...
        Console.WriteLine(
            $"No newer version available; latest released on Github is {latestVersion}.");
      }
    }
    ...
    public Version GetLatestVersion(string repoTagsRaw) {
      ...
      var item = latestRelease.Children<JProperty>().FirstOrDefault(
          p => p.Name == "name");
      ...
      if (!matches.Any()) {
        Console.WriteLine(
            $"Couldn't parse downloaded release tag '{latestReleaseTag}' to extract the version.");
        return null;
      }
      ...
      if (!Version.TryParse(latestReleaseVersion, out latestVersion)) {
        Console.WriteLine(
            $"Couldn't parse latest downloaded version '{latestReleaseVersion}' into structured data.");
      }
      ...
      using(var client = new HttpClient()) {
        ...
        using(var requestMessage =
          ...
          try {
            ...
            Console.WriteLine(
                $"Failed to download latest Hammer release tags: {exception.Message}");
            return null;
          }
  ...
  public class CLIParser {
    ...
    public class CLIOptions {
      [Option('s', "scopedir", Required = false,
              HelpText = "Set the scope directory")]
      public string ScopeDir {
        get;
        set;
      }

      [Value(0, MetaName = "resource_file",
             HelpText = "The resource file to validate")]
    ...
    /// <param name="args">The list of command line arguments as passed to the
    /// application</param>
    /// </summary>
    ...
    public void Process() {
      _cliOptions.WithParsed(result => {
        ...
        if (result.ScopeDir != null) {
          var scopeUri =
              new System.Uri(System.IO.Path.GetFullPath(result.ScopeDir));
          AppModel.Instance.LoadScopeDirectory(scopeUri.ToString());
        }
        if (result.ResourceFile != null) {
          var resourceUri =
              new System.Uri(System.IO.Path.GetFullPath(result.ResourceFile));
  ...
  static int Main(string[] args) {
    ...
    QCoreApplication.SetAttribute(ApplicationAttribute.EnableHighDpiScaling,
                                  true);
